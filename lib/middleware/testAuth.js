...
const checkTestAuth = (req, res, next) => {
  ...
  // Only allow test auth when explicitly enabled
...
const generateTestTokens = () => {
  ...
  if (!isTestAuthEnabled) {
    throw new Error('Test token generation not allowed - ALLOW_TEST_AUTH must be set to true');
  }
...
const testTokenEndpoint = (req, res) => {
  ...
  // Only allow when test auth is enabled
  if (!isTestAuthEnabled) {
    return res.status(403).json({ error: 'Test tokens not available - ALLOW_TEST_AUTH must be set to true' });
  }
